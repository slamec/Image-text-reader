...
import pytesseract #possible to use also PDF and so on - click on pytesseract
...
def Browse_button():
    ...
    Browse_button.directory = filedialog.askopenfilename(initialdir = 'C:/Users/%s', 
                                            title = "Please select a file",
                                            filetypes =  (("All","*.png *.jpeg .*gif"),
                                                        ("png files","*.png"),
                                                        ("jpeg files","*.jpeg"),
                                                        ("gif files", ".gif")))
    return Browse_button.directory
...
def Image_to_text():
    ...
    image = cv2.imread(Browse_button.directory)

    Image_to_text.st = pytesseract.image_to_string(image, lang='eng+ces', config='--psm 1')
        #important to get file with proper encoding

    return Image_to_text.st
